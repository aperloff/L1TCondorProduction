...
CMSSWVER=CMSSW_11_1_7
FORK=aperloff
BRANCH=phase2-l1t-integration-${CMSSWVER}
ACCESS=ssh
CORES=8
NAME=""
DIR="${PWD}"
...
if [[ -n "$BATCH" ]]; then
	...
	git config --global --add user.github "L1T"
fi
if [[ "${DEBUG}" == "true" ]]; then
...
git clone ${ACCESS_GITHUB}${FORK}/L1TCondorProduction.git -b ${BRANCH} Condor/L1Trigger
